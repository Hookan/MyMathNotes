...
>
> Sometimes, we will see $\Delta^2$ as $I$ with $(1-t,t)\mapsto t$.
...
> **1.5 Definition.**
>
> Let $X$ be a topological space, $n\ge 0$. A **(singular) $n$-simplex** in $X$ is a continue function $f:\Delta^n\to X$.
>
...
>
> Since $j\sigma_i=j\sigma_j\Rightarrow \sigma_i=\sigma_j$, we have $j\sigma_i$ are distinct. Therefore, for each $i$, $m_i=0$. $\square$
...
>   $$
>   \begin{align}
>   \partial\varphi_n(\sum_i m_i \sigma_i)&=\partial(\sum_i m_if_{\lambda, n}j_{\lambda_i\#}(\sigma_i))\\
>   &=\sum_im_i\partial f_{\lambda, n}j_{\lambda_i\#}^{-1}(\sigma_i)\\
>   &=\sum_im_if_{\lambda, n} \partial j_{\lambda_i\#}^{-1}(\sigma_i)\\
>   &=\sum_im_if_{\lambda, n}j_{\lambda_i\#}^{-1}\partial(\sigma_i)\\
...
>   }
>   $$
>   , we have $\sum  j_\lambda b_\lambda=b=\partial\gamma = \partial \sum j_\lambda\gamma_\lambda=\sum \partial j_\lambda\gamma_\lambda=\sum j_\lambda(\partial\gamma_\lambda)$. Consequently, we have $\sum j_\lambda(b_\lambda-\partial\gamma_\lambda)=0$.
>
>   Therefore, we have $b_\lambda=\partial\gamma_\lambda$; hence $\varphi(\operatorname{cls} b)=\sum f_\lambda(\operatorname{cls}b_\lambda)=0$.
>
>* Uniqueness of $\varphi$:
>
>   If there are $\varphi$ and $\psi$ both satisfy the properties, then we have
...
>
> Define $B=\{\sum m_i x_i;\sum m_i=0\}$. It's obvious that $B$ is a subgroup of $S_0(X)$.
>
...
>
>   And then, we have $\partial\gamma=\sum (a_i\sigma_i\varepsilon_0-a_i\sigma\varepsilon_1)=\sum m_i x_i$.
>
>   Since $x_i,\sigma_i\varepsilon_0,\sigma_i\varepsilon_1$ are generators, $\sum m_i=\sum (a_i-a_i)=0$.
>
>   Therefore, $\sum m_i x_i\in B$.
>
> * Let $\sum m_i x_i\in B$, and fix a point $p\in X$.
>
>   Let $\sigma_x:\Delta^1\to X$ be a path from $p$ to $x$ (consider $\Delta^1$ as $I$).
>
>   Then let $\gamma=\sum m_i\sigma_{x_i}\in S_1(X)$. We can find that
>   $$
>   \begin{align}
>   \partial\gamma&=\sum m_i(\sigma_{x_i}\varepsilon_0-\sigma_{x_i}\varepsilon_1)\\
>   &=\sum m_i(x_i-p)\\
>   &=-(\sum m_i)p+\sum m_ix_i\\
>   &=\sum m_ix_i.
>   \end{align}
>   $$
...
>   If $(*)$ is true, we have
>   $$
>   \partial P_n^{\Delta}(\delta)=\lambda_{0\#}^X(\delta)-\lambda_{1\#}^X(\delta)-P_{n-1}^\Delta\partial(\delta).
>   $$
>   Therefore, we need to prove $(\lambda_{0\#}^X-\lambda_{1\#}^X-P_{n-1}^\Delta\partial)(\delta)\in B_n(\Delta^n\times I)$.
>
...
>
>   Consequently, we only need to prove $(\lambda_{0\#}^X-\lambda_{1\#}^X-P_{n-1}^\Delta\partial)(\delta)\in Z_n(\Delta^n\times I)$​.
>
>   Since
>   $$
>   \begin{align}
>   &\partial(\lambda_{0\#}^X-\lambda_{1\#}^X -P_{n-1}^\Delta\partial )\\
>   =&\partial\lambda_{0\#}^X -\partial\lambda_{1\#}^X -\partial P_{n-1}^\Delta\partial \\
>   =&\partial\lambda_{0\#}^X -\partial\lambda_{1\#}^X -(\lambda_{0\#}^X -\lambda_{1\#}^X -P_{n-1}^\Delta\partial)\partial &&(\text{by inductive hypothesis})\\
>   =&\lambda_{0\#}\partial -\lambda_{1\#}^X\partial -(\lambda_{0\#}\partial -\lambda_{1\#}^X\partial )+P^\Delta_{n-1}\partial\partial \\
>   =&0,
>   \end{align}
>   $$
>   we have $(\lambda_{0\#}^X-\lambda_{1\#}^X-P_{n-1}^\Delta\partial)(\delta)\in B_n(\Delta^n\times I)$; that is $\exists \beta_{n+1}\in S_{n+1}(\Delta\times I)$, such that $\partial\beta_{n+1}=(\lambda_{0\#}^X-\lambda_{1\#}^X-P_{n-1}^\Delta\partial)(\delta)$​.
>
>   Define
>   $$
